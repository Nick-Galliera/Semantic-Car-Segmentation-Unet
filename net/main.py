...
if __name__ == "__main__":
    ...
    preprocess_input = get_preprocessing_fn(
        encoder_name=PRETRAINED_BACKBONE, pretrained="imagenet"
    )
    ...
    """ soft augmentations: only rotations both imgs and masks """
    if AUGMENT:
        augmented_indices = random.sample(
            range(len(train_dataset)), int(AUGMENT_SIZE * len(train_dataset))
        )
        ...
        for image, mask in subset_for_augmentation:
            ...
            augmented_image = T.functional.rotate(
                image, angle=rotation_angle, interpolation=T.InterpolationMode.BILINEAR
            )

            augmented_mask = mask.unsqueeze(0)
            augmented_mask = T.functional.rotate(
                augmented_mask,
    ...
    if SHOW_DATASET_PREVIEW:
        get_dataset_inspection(
            dataset=train_dataset, json_dir=JSON_FILE, index=len(train_dataset) - 1
        )
    ...
    pretrained_model = Pretrained_U_net(
        encoder_name=PRETRAINED_BACKBONE,
        requiregrad_encoder=False,
    ...
    """
    if TENSORBOARD_VISUALIZE:
        img_tbord , _ = next(iter(train))
        tensorboard.plot_graph(pretrained_model, img_tbord)
    """

    class_weights = torch.tensor([1.0, 1.0, 1.0, 5.0, 1.0]).to(DEVICE)
    pretrained_criterion = torch.nn.CrossEntropyLoss(weight=class_weights)

    pretrained_optimizer = optim.Adam(
        pretrained_model.model.decoder.parameters(), lr=PRETRAINED_LR
    )

    pretrained_lrscheduler = ReduceLROnPlateau(
        pretrained_optimizer, "min", patience=5, factor=0.1, verbose=True
    )
    ...
    if TRAIN_PRETRAINED:
        train_model(
            ...
            scheduler=pretrained_lrscheduler,
        )
        torch.save(pretrained_model.state_dict(), PRETRAINED_WEIGHTS)
    else:
        print(f"[?] Loading {PRETRAINED_WEIGHTS}")
        pretrained_model.load_state_dict(
            torch.load(PRETRAINED_WEIGHTS, map_location=DEVICE, weights_only=True)
        )

    if TEST_PRETRAINED:
        inference_model(model=pretrained_model, val_loader=test, show_samples=True)
